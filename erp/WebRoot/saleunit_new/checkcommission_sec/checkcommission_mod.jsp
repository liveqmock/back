    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $('#tbList').datagrid({
                ...
                url:'',
                columns:[[
                    {field:'ck',title:"选项",checkbox:true,hidden:false,width:$(this).width() * 0.2},
                    {field:'unit_id',title:'单元ID',hidden:true,width:$(this).width() * 0.2},
                    {field:'checkfeeid',title:'checkfeeid',hidden:true,width:$(this).width() * 0.2},
                    {field:'checkcommissionId',title:'checkcommissionId',hidden:true,width:$(this).width() * 0.2},
                    {field:'checkcommissionTypeName',title:'状态',align:"center",hidden:false,width:$(this).width() * 0.2},
                    {field:'checkcommission_date',title:'对佣日期',align:"center",hidden:false,width:$(this).width() * 0.2},
                    {field:'work_date',title:'成交日期',align:"center",hidden:false,width:$(this).width() * 0.2},
                    {field:'sign_date',title:'签约日期',align:"center",hidden:false,width:$(this).width() * 0.2},
                    {field:'area_name',title:'分区',hidden:false,width:$(this).width() * 0.2},
                    {field:'build_name',title:'楼栋',hidden:false,width:$(this).width() * 0.2},
                    {field:'unit_no',title:'单元号',hidden:false,width:$(this).width() * 0.2},
                    {field:'customer_name',title:'业主姓名',sortable:true,hidden:false,width:$(this).width() * 0.2},
                    {field:'build_area',title:'建筑面积(㎡)',sortable:true,align:"right",hidden:false,width:$(this).width() * 0.2},
                    {field:'sum_price',title:'成交总价(元)',sortable:true,align:"right",hidden:false,formatter:formatPrice,width:$(this).width() * 0.2},
                    {field:'contract_no',title:'合同号',hidden:false,width:$(this).width() * 0.2},
                    {field:'pay_name',title:'付款方式',hidden:false,width:$(this).width() * 0.2},
                    /*{field:'commission',title:'佣金点(%)',align:"center",editor:'text',formatter:formatPrice,width:$(this).width() * 0.2},
                    {field:'commissionAmount',title:'应收佣金(元)',align:"center",hidden:false,formatter:formatPrice,editor:'text',width:$(this).width() * 0.2},*/
                    {field:'sec_commission',title:'一二手佣金点(%)',align:"center",editor:'text',formatter:formatPrice,width:$(this).width() * 0.2},
                    {field:'sec_commissionAmount',title:'一二手应收佣金(元)',align:"center",hidden:false,formatter:formatPrice,editor:'text',width:$(this).width() * 0.2},
                    /*{field:'rel_commission',title:'关系户佣金点(%)',width:60,align:"center",editor:'text',formatter:formatPrice},
                    {field:'rel_commissionAmount',title:'关系户应收佣金(元)',width:60,align:"center",hidden:false,formatter:formatPrice,editor:'text'},*/
                    ...
                    {field:'sales',title:'销售人员',hidden:false,width:$(this).width() * 0.2}
        ...
        function getChanges(){
            ...
            for(var i=0;i<rows.length;i++) {
                ...
                //savecommission
                var commission,commissionAmount,sec_commission,sec_commissionAmount,rel_commission,rel_commissionAmount ;
                ...
                if(rows[i].sec_commission==null || rows[i].sec_commission=="-" || rows[i].sec_commission.length==0){
                    sec_commission = "0";
                } else {
                    sec_commission = rows[i].sec_commission;
                }
                if(rows[i].sec_commissionAmount==null || rows[i].sec_commissionAmount=="-" || rows[i].sec_commissionAmount.length==0){
                    sec_commissionAmount = "0";
                } else {
                    sec_commissionAmount = rows[i].sec_commissionAmount;
                }
                ...
                if(rel_commission==0 && rel_commissionAmount==0){
                    if(parseFloat(commissionAmount) + parseFloat(sec_commissionAmount)==0){
                        myAlert("佣金不能为 0");
                        return ;
                    }
                } else {
                    if(parseFloat(rel_commissionAmount) + parseFloat(sec_commissionAmount)==0){
                        myAlert("佣金不能为 0");
                        return ;
                    }
                }
                ...
                sec_commissions.push(sec_commission);
                sec_commissionAmounts.push(sec_commissionAmount);
            ...
            if(isChange){
                $.ajax({
                    type: 'get',
                    url: './saleunit_new/checkcommission_sec/savecommission.action',
        ...
        function exportExcel(){
            ...
            location.href = './saleunit_new/checkcommission_sec/exportExcel_Mod.action?projectIds='+projectIds +"&checkCommissionDate="+checkCommissionDate;
        }
		...
		function addUnit(){
			...
			$.ajax({
				type:"get",
				url: './saleunit_new/checkcommission_sec/getCheckCommissionRefundType.action',
				...
				success: function(data){
					...
					/*if(type == "1"){
						...
						//部分
						partRefund(checkCommissionDate, projectIds);
					}else{
						myAlert("返回类型有异常");
					}*/
				},
		...
		function allRefund(checkCommissionDate, projectIds,repayType){
			...
			new MyAjaxIframeDialog({title:'新增单元选择列表',height:550, width:800,
				//src:'./saleunit/operation/chooseNewUnit.action?type=checkfeeForAllRefund&propertyProjectId=' + projectIds,
				src:'./saleunit_new/checkcommission_sec/checkcommission_addUnit.action?propertyProjectId=' + projectIds +"&repayType="+repayType,
				submitFn:function(){
					...
					$.ajax({
						type:"get",
						url: './saleunit_new/checkcommission_sec/add_checkcommission.action',
        ...
        function getCheckCommissionDate(projectId){
			...
			$("#checkCommissionDate").combobox(
                    "reload",
                    "./saleunit_new/checkcommission_sec/getDateListForCombotree.action?propertyId=" + projectId +"&checkCommissionType=" + checkCommissionType
            );
        	}
        }
        /**
