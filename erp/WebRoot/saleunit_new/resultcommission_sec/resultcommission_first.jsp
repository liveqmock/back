...
$(document).ready(function(){
    ...
    $('#tbList').datagrid({
        ...
        loadMsg:'数据加载中...',
        url:'',autoSizeColumn:true,
        columns:[[
            {field:'ck',title:"选项",checkbox:true},
            ...
            {field:'sec_commissionAmount',title:'应收款(元)',width:70,align:"center"},
            {field:'sec_commission',title:'应收佣金点(%)',width:70,align:"center"},

            {field:'work_date',title:'成交日期',width:70,align:"center"},
            {field:'sign_date',title:'签约日期',width:70,align:"center"},
            {field:'area_name',title:'分区',hidden:true},
            ...
            {field:'sec_commissionAmount',title:'一二手应收佣金(元)',width:60,align:"center"},
            {field:'sales',title:'销售人员',width:80},
            {field:'checkcommissionTypeName',title:'状态',align:"center",hidden:true},
            {field:'checkcommission_date',title:'对佣日期',align:"center",hidden:true}
        ]],
    ...
    function totalCommission(val,row){
        ...
        if(rel_commission==0 && rel_commissionAmount==0 ){
            val = commission + sec_commission;
        } else {
            val = rel_commission + sec_commission;
        }
   ...
   // var sum_paymentAmount = 0;
    var i=0;
    function totalCommissionAmount(val,row){
        var sec_commissionAmount = row.sec_commissionAmount;
        var commissionAmount = row.commissionAmount;
        var rel_commissionAmount = row.rel_commissionAmount;
        var rel_commission = row.rel_commission;

        if(sec_commissionAmount==null || sec_commissionAmount=="" || sec_commissionAmount=="undefined"){
        ...
        if(rel_commission==0 && rel_commissionAmount==0){
            val = parseFloat(commissionAmount) + parseFloat(sec_commissionAmount);
        } else {
            val = parseFloat(rel_commissionAmount) + parseFloat(sec_commissionAmount);
        }
...
function exportExcel(){
    ...
    location.href = './saleunit_new/resultcommission_sec/exportResultCommission.action?projectIds='+projectIds
            +"&checkCommissionDate="+checkCommissionDate;
...
function compareToCommissionAmount(colname){
    var ids = [];
    var rows = $('#tbList').datagrid('getSelections');
    var sec_commissionAmount = 0;
    var paymentAmount = $("#paymentAmount").val();
    $.each(rows, function (i, obj) {
        $.each(obj, function (name, val) {
            if(name==colname){
                //ids.push(val);
                sec_commissionAmount = sec_commissionAmount+val;
            }
        });
    });
    if(paymentAmount > sec_commissionAmount) {
        return false;
    }
    return true;
}
...
function getCheckCommissionDate(projectId){
    ...
    $("#checkCommissionDate").combobox(
            "reload",
            "./saleunit_new/checkcommission_sec/getDateListForCombotree.action?propertyId=" + projectId
            // +"&checkCommissionType="+$("#checkCommissionType").combobox("getValue")
    );
	}
}
...
function btn_save() {
    var ids = getSelections("unit_id");
    var checkcommissionIds = getSelections("checkcommissionId");
    var t_commissionAmounts = getSelections("sec_commissionAmount");
    var projectIds = $("input[name='propertyUnitCond.strSearchProjectIds']").val();
    ...
    if(!compareToCommissionAmount("sec_commissionAmount")){
        myAlert("实收金额不能大于应收款。");
        return;
    }
    ...
    $.ajax({
        type: 'get',
        url: './saleunit_new/resultcommission_sec/savePayment.action',
