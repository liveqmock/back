...
$().ready(function(){
	...
	*/
	...
	$("#projectName").autocomplete("./customer_guangzhou/search/projects.action", {
		...
		setHiddenCompanyId: "hiddenCompanyId",
		selectFunIsArg: "true"
	});
...
//			$("#ques_table").load('./customer_guangzhou/input/findQuestionTable.action',{proId:pro_aja_id});
//			//val_jsp_and_proid(pro_aja_id);
...
function validateCustForm(){
		...
		$.ajax({
			type:"post",
			async:false,
			url:"./customer_guangzhou/input/validateQuestion.action?"+thisform,
			dataType:"json",
			success:function(data){
				if(data!=null){
...
function setSomeSel(projectId){
	if(projectId != "" && projectId != "0"){
		...
		$.ajax({
			type:"get",
			url: './customer_guangzhou/search/getSelProjectDefaultProvince.action?projectId=' + projectId,
			dataType: "html",
			success: function(data){
				$("#customer_homeProvince").val(data);
				$("#customer_workProvince").val(data);
			}
		}); 
		
		//设置居住区域,工作区域的默认城市
		setSelByIdAndAction("customer_homeCity", "./customer_guangzhou/search/getSelProjectDefaultCity.action", "projectId=" + projectId);	
		setSelByIdAndAction("customer_workCity", "./customer_guangzhou/search/getSelProjectDefaultCity.action", "projectId=" + projectId);		
		
		//设置居住区域,工作区域的默认区下拉框
		setSelByIdAndAction("customer_homeRegion", "./customer_guangzhou/search/getSelProjectDefaultRegion.action", "projectId=" + projectId);
		setSelByIdAndAction("customer_workRegion", "./customer_guangzhou/search/getSelProjectDefaultRegion.action", "projectId=" + projectId);
