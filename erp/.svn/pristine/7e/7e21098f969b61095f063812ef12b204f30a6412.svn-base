...
<head>
	...
	<script type="text/javascript" language="javascript">
		...
		function formSubmit(){
			...
			$("#updateDiscountManagerFormId").submit();
		...
		function loadClose(){
			...
			var updateLoadClose = '${updateLoadClose}';
			
			if(updateLoadClose == "true"){
				var unitDiscountId = $("#unitDiscountId").attr("value");
				window.setTimeout(function(){window.parent.closeUpdateDiscount(unitDiscountId);}, 1000);				
			}
...
<form action="./saleunit_new/appoint/guangzhou/updateDiscountManager.action" method="post" id="updateDiscountManagerFormId">
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="gbox" style="font-size:12px; line-height:26px; white-space:nowrap">
                ...
                <td id="t16" style="" colspan="2">	
					...
					<table style="width:100%; white-space:nowrap">
						...
						<!--
							...
							<td><input style="width:80px" type="text" name="unitDiscount.addUnitPrice" value="${unitDiscount.addUnitPrice}" class="price" /></td>
						-->
			  ...
			  <tr  onMouseOver="this.style.backgroundColor='#f1f9fe'" onMouseOut="this.style.backgroundColor=''" bgcolor="#FFFFFF" 
				...
				<td colspan="3" width="100%" height="100%">
					...
					<input type="hidden" name="unitDiscount.confirmType" value="${unitDiscount.confirmType}" />
					<input type="hidden" name="unitDiscount.mainId" value="${unitDiscount.mainId}" />
					<input type="hidden" name="unitDiscount.unitId" id="hiddenUnitId" value="${unitDiscount.unitId}" />
					<input type="hidden" name="unitDiscount.buildId" value="${unitDiscount.buildId}" />
					<input type="hidden" name="unitDiscount.id" value="${unitDiscount.id}" id="unitDiscountId"/>
					<input type="hidden" name="unitDiscount.discountName" value="${unitDiscount.discountName}" />
