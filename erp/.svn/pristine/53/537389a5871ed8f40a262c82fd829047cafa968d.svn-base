...
function Dialog(content, options)
{
    var defaults = { // 默认值。 
        ...
        closeText:'<img src="js/X.gif" width="16" height="16" />', // 关闭按钮文字，若不想显示关闭按钮请通过CSS设置其display为none 
        draggable:true,     // 是否移动 
        modal:true,         // 是否是模态对话框 
        center:true,        // 是否居中。 
    ...
    var init = function()
    {
		...
		//我加的
		//dialog.find('.my').bind('click', this.close);
		dialog.find('.my')
		.bind('click', function(){
			//customerName customerNameSpan customerPwd customerPwdSpan
			var customerName = $("#customerName").val();
			var customerPwd = $("#customerPwd").val();	
			
			if(customerName == ""){
				$("#customerNameSpan").html("<img src='js/empty.gif'/>");
				return false;
			}
			
			if(customerPwd == ""){
				$("#customerPwdSpan").html("<img src='js/empty.gif'/>");
				return false;
			}
			
			$.ajax({
				type:"get",
				url: "./login!customerLogin.action",
				data: "name=" + customerName + "&pwd=" + customerPwd,
				dataType: "html",
				success: function(data){
					if(data != ""){
						//服务器端返回成功登陆的customer名称,并在session中保存,因在js传中文过去会有乱码,所以把"欢迎, "也放在服务器端返回
						
						$("#loginElse").hide();
						$("#welcomeTitleElse").html(data);
						$("#welcomeTitleElse").show();
			...
			//要remove掉才能使得新弹出的窗口不会引用旧的值
			$("div").remove(".dialog-overlay");			
			$("div").remove(".dialog");
			
			//下面的方法会使的新的窗口引用旧的值
			//dialog-5-overlay去掉数字后面的内容,就可以作为id
			//var oId = overlayId.split("-");
			//var id = oId[0] + "-" + oId[1];
			//$("#" + id).hide();	
