...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(document).ready(function() {
			bindCompanyForSQKHOnlyQuestion("companyNames", "companyIds"); //单个公司的选择	
			
		});			
								
		function download(){
			location.href = './saleunit_new_report/report/guangzhou/customerQuestionCompanyDownload.action';	   
		}		
		...
		function queryForm(){
			...
			$("#dg").datagrid({
				url:"./saleunit_new_report/report/guangzhou/customerQuestionCompanyAjax.action",
				queryParams:getInputsAsOjbect(["cond.strSearchCompanyIds","cond.date1","cond.date2","cond.sort","cond.order","questionId"])
			});
		}				
