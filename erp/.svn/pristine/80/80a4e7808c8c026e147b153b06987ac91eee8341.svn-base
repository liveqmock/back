 ...
 */
public class ChartCustomerNumCompanyAction extends SupperAction {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(ChartCustomerNumCompanyAction.class);

	@Autowired
	IReportPreCustomerServices reportPreCustomerServices;
	
	private ReportPreCustomerCompanyDateCond cond;
	
	//数据库分组结果
	private List<ReportPreCustomerCompanyDate> listDBData;
	...
	public String searchAjax() throws Exception {
		...
		ActionTemplate.executeAjaxMethod(this, new ActionAjaxMethodModifyNoThrowsExceptionCallback() {			
			...
			public void modifyMethod() throws Exception {
				...
				listDBData = reportPreCustomerServices.groupByCompanyDate(cond);
	...
	private String getChartData(){
		...
		for (int i=0;i<listDBData.size(); i++) {
			...
			String key = "";
			String projectName = listDBData.get(i).getCompanyName();
			...
			if(selCycel.equals(EnumChartCycel.WEEK.toString().toLowerCase())){
				...
				if(key.substring(1, 2).equals("-")){
					key = listDBData.get(i).getCompanyName()+":"+"0"+key;
				}
			}
			else if(selCycel.equals(EnumChartCycel.MONTH.toString().toLowerCase())){
				key = listDBData.get(i).getCompanyName()+":"+DateTimeUtils.getMonthFirstDayStrMMdd(listDBData.get(i).getReportDate().toString());
			}
			else{
				key = listDBData.get(i).getCompanyName()+":"+listDBData.get(i).getReportDate().substring(5);
			}
	...
	public ReportPreCustomerCompanyDateCond getCond() {
		return cond;
	}

	public void setCond(ReportPreCustomerCompanyDateCond cond) {
		this.cond = cond;
	}
