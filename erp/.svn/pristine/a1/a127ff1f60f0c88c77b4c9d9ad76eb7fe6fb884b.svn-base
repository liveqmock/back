 ...
 *
 */
public class CustomerFollowCompanyAction extends SupperAction {
	...
	private void setChartSeriesDataInSession(List<Map<String, Object>> dataList) {
		...
		for (Map<String,Object> p : dataList) {
			str_Phone += "['" + p.get("companyName") + "'," + p.get("phoneCount") + "]"
					+ ",";

			str_Visit += "['" + p.get("companyName") + "'," + p.get("visitCount") + "]"
					+ ",";
			str_Purchased += "['" + p.get("companyName") + "'," + p.get("purchasedCount") + "]"
					+ ",";
			str_Phone2visit += "['" + p.get("companyName") + "'," + p.get("phone2visitCount") + "]"
					+ ",";
			str_IdentificationChips += "['" + p.get("companyName") + "'," + p.get("identificationChipsCount") + "]"
					+ ",";
			str_Initiation += "['" + p.get("companyName") + "'," + p.get("initiationCount") + "]"
					+ ",";
			str_HomeExhibitionsWill += "['" + p.get("companyName") + "'," + p.get("homeExhibitionsWillCount") + "]"
					+ ",";
			str_ExternalExhibition += "['" + p.get("companyName") + "'," + p.get("externalExhibitionCount") + "]"
					+ ",";
			str_Leaflet += "['" + p.get("companyName") + "'," + p.get("leafletCount") + "]"
					+ ",";
		}
		...
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PHONE+ContSessionAttribute.TITLE, "客户跟进情况(公司)-电话跟进");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PHONE+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Phone);

		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_VISIT+ContSessionAttribute.TITLE, "客户跟进情况(公司)-再次到访");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_VISIT+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Visit);
		
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PURCHASED+ContSessionAttribute.TITLE, "客户跟进情况(公司)-已购买");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PURCHASED+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Purchased);
		
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PHONE2VISIT+ContSessionAttribute.TITLE, "客户跟进情况(公司)-电转访");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_PHONE2VISIT+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Phone2visit);
		
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_IDENTIFICATIONCHIPS+ContSessionAttribute.TITLE, "客户跟进情况(公司)-认筹");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_IDENTIFICATIONCHIPS+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_IdentificationChips);
		
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_INITIATION+ContSessionAttribute.TITLE, "客户跟进情况(公司)-入会");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_INITIATION+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Initiation);

		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_HOMEEXHIBITIONSWILL+ContSessionAttribute.TITLE, "客户跟进情况(公司)-房展会");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_HOMEEXHIBITIONSWILL+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_HomeExhibitionsWill);

		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_EXTERNALEXHIBITION+ContSessionAttribute.TITLE, "客户跟进情况(公司)-外展场");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_EXTERNALEXHIBITION+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_ExternalExhibition);

		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_LEAFLET+ContSessionAttribute.TITLE, "客户跟进情况(公司)-派单");
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY_LEAFLET+ContSessionAttribute.CHART_SERIES_DATA, chartSeriesData_Leaflet);
	}
	...
	private void setDownloadDataInSession(List<Map<String, Object>> dataList){
		request.getSession().setAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY+ContSessionAttribute.DOWNLOAD_DATA, dataList);		
	}
	...
	public String download() throws Exception {			
		...
		List<Map<String, String>> dataList = (List<Map<String, String>>) request.getSession().getAttribute(ContSessionAttribute.CUSTOMER_FOLLOW_COMPANY+ContSessionAttribute.DOWNLOAD_DATA);

		Map<String, Object> map = new HashMap<String, Object>();
		map.put("dataList", dataList); //与模板里面的循环要求对应
		
		ReportUtils.downLoadReport(map, "customer-follow-company.xls", "download-客户跟进情况-公司-" + CustomerUtils.getNowForString() + ".xls", response);
	...
	public String searchAjax() throws Exception {
		...
		ActionTemplate.executeAjaxPage(this, cond, new ActionAjaxPageByFooterCallback() {
			...
			public List<Map<String, Object>> initRows() throws Exception {
				...
				List<Map> list = customerFollowServices.findReportSumGroupByCompany(cond); 
				if(!CommonUtils.isCollectionEmpty(list)){
					...
					for(Map obj : list){
						...
						map.put("companyId", obj.get("companyId").toString());
						map.put("companyName", obj.get("companyName").toString());
