...
public class SaleReportAction extends SupperAction {
	...
	private ReportPojoXSFXByProjectCond cond;
	...
	public String searchAjax() throws Exception {
		if(cond==null){
			cond = new ReportPojoXSFXByProjectCond();
		}
		...
		ActionTemplate.executeAjaxPage(this, cond, new ActionAjaxPageCallback() {
			...
			public List<Map<String, String>> initRows() throws Exception {
				...
				List<ReportPojoXSFXByProject> list ;
				if(selType.equalsIgnoreCase("contract")){
					list = reportUnitSaleServices.findXSFXByProject_Contract(cond);					
				}
				else if(selType.equalsIgnoreCase("confirm")){
					list = reportUnitSaleServices.findXSFXByProject_Confirm(cond);					
				}
				else{
					list = reportUnitSaleServices.findXSFXByProject_ConfirmBook(cond);
				}
				...
				if(!CommonUtils.isCollectionEmpty(list)){
					...
					for(ReportPojoXSFXByProject obj : list){
	...
	private void setDownloadDataInSession(List<Map<String, String>> dataList){
		request.getSession().setAttribute(ContSessionAttribute.XSFX_XM+ContSessionAttribute.DOWNLOAD_DATA, dataList);		
	}
		
	public ReportPojoXSFXByProjectCond getCond() {
		return cond;
	}


	public void setCond(ReportPojoXSFXByProjectCond cond) {
		this.cond = cond;
	}
	...
	public String download() throws Exception {			
		...
		List<Map<String, String>> dataList = (List<Map<String, String>>) request.getSession().getAttribute(ContSessionAttribute.XSFX_XM+ContSessionAttribute.DOWNLOAD_DATA);

		Map<String, Object> map = new HashMap<String, Object>();
		map.put("dataList", dataList); //与模板里面的循环要求对应
		
		ReportUtils.downLoadReport(map, "xsfx-xm.xls", "download-销售分析(项目)-" + CustomerUtils.getNowForString() + ".xls", response);
